...
char* indexes(const char* path)
{
    int pathLength = strlen(path); // Amount of letters i path
    int lenghtOfindexPhp = 9 + 1; // amount of letter in index.php plus terminator
    char* indexPathMA = malloc(pathLength + lenghtOfindexPhp); // Memory allocatioon for full index path
    if (indexPathMA == NULL)
    {
        printf("Memory allocation error.\n");
        return NULL;
    }
    strcpy(indexPathMA, path); 
    strcat(indexPathMA, "index.php");
    
    // check if index.php exists in path
    if (access(indexPathMA, F_OK) != -1)
    {
       return indexPathMA;
    }
    else
    ...
    int lenghtOfindexHtml = 10 + 1; // amount of letter in index.html plus terminator
    char* indexPathMAHtml = malloc(pathLength + lenghtOfindexHtml);
    if (indexPathMAHtml == NULL)
    {
        printf("Memory allocation error.\n");
        return NULL;
    }
    strcpy(indexPathMAHtml, path);
    strcat(indexPathMAHtml, "index.html");
    
    if (access(indexPathMAHtml, F_OK) != -1)
    {
       return indexPathMAHtml;
    }
    else
    {
        free(indexPathMAHtml);
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while (fread(buffer, buffer_size, num_elements, file) != 0) //while there is data in the document
    {
        *content = realloc(*content, *length + buffer_size + 1); //reallocate memory to the amount of bytes plus buffer plus 1
        if (*content == NULL) //if no bytes return false
        {
            printf("Memory allocation error.\n");
            *length = 0;
            return false;
